...
train_imgs, val_imgs, train_labels, val_labels = get_train_val_split(
    os.path.join(config.INFO_DIR, "train_test_split.csv")
)

# 2: Prepare dataset
transform = CenterCrop([int(config.IMG_HEIGHT / 2), int(config.IMG_WIDTH / 2)])
val_set = CTDataSet(
    val_imgs, val_labels, config.VOLUMES_DIR, config.ORGAN_TYPE, transform
)

# 3: Prepare validation dataloader
val_data_loader = DataLoader(
    val_set, batch_size=1, shuffle=True, num_workers=0, pin_memory=True
)

# 4: Load Unet and state dictionary
model = Unet(config.NUM_CLASSES).to(config.DEVICE)
model.load_state_dict(
    torch.load(os.path.join(config.MODEL_DIR, "All_model_100epochs_10batch_71pts.pt"))
)
model.eval()
...
pred_list = []
progressbar = tq.tqdm(
    val_data_loader, desc="Evaluating model...", total=len(val_data_loader)
)
with torch.no_grad():
...
results_display(
    display_gt_bottom, display_pred_bottom, display_scores_bottom, type="worst"
)
